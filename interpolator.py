...
def interpolate_array(arrays_dict, needed_date):
    ...
    if (after - before).days == 0:
        result = ar0  # needed date is one of the dates in input_arrays and does not need to be interpolated
    else:
        result = ar0 + (needed_date - before).days * (ar1 - ar0) / (after - before).days  # linear interpolation
...
def _get_closest_dates(arrays_dict, needed_date):
    ...
    dates = [datetime.strptime(date_str, "%Y%m%d") for date_str in
             [*arrays_dict]]  # [*arrays_dict] returns a list containing keys from input dict
